...
if(isset($_POST['change']))
{
	...
	if($password==$cpassword)
	{
		...
		$query =mysqli_query($conn,"UPDATE student SET password='$md5p' WHERE id='$std_id'");
			if($query)
		 {
                   session_unset();
...
$query2 =mysqli_query($conn,"SELECT * FROM student WHERE id='$std_ids'");
			$row2 = mysqli_fetch_array($query2);
		echo "<p class='text-center text-danger'>Your Student Id is ".$row2['student_id']."</p>";
?>
